                    ...
                    <div class="card invoice-preview-card">
                            ...
                            <div class="d-flex justify-content-between flex-xl-row flex-md-column flex-sm-row flex-column m-sm-3 m-0">
                                <div class="mb-xl-0 mb-4">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        <button v-if="category.sub_category" type="button" class="btn btn-outline-primary waves-effect" @click="getAll()">All</button>
                                        <button v-for="(sub, index) in category.sub_category" @click="filter(sub)" :key="index" type="button" class="btn btn-outline-primary waves-effect">{{ sub.name }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive border-top">
                            <table class="table m-0">
                                ...
                                <tbody>
                                    <tr v-for="(sub, index) in get_asset" :key="index">
...
export default {
    ...
    methods: {
        ...mapActions(['fetch_category_products', 'filter_asset_product']),
        filter: async function(sub) {
            let data = {
                category_id: this.$route.params.id,
                sub_category_id: sub.id
            }
            await this.filter_asset_product(data)
        },
    ...
    computed: {
        ...mapGetters(['get_asset_category_single_data', 'get_asset']),
        
        category: function () {
    
          return this.get_asset_category_single_data
        }
